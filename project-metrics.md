...
> ⚠️ **Every number below this line was measured on the pre-2026-08-17 benchmark and is superseded.** The data pipeline was rebuilt on 2026-08-17 to fix four defects (benign duplicated 54-158x, cross-task benign leakage, a 2.9x task-size imbalance, and a relation-aware EWC mechanism that could only modulate 2.2% of its own penalty) — see `CLAUDE.md`'s "Project state" section and `docs/methodology-2026-08-17.md`. **Nothing in this file is comparable to a number produced after that date**, including the "Benchmark at a glance" table immediately below. `docs/results.md` (generated by `trench_ids.cl.report`) is the current source of truth for CL metrics; this file (last updated 2026-07-30, through §30) is kept as the historical record of the pre-rebuild benchmark, not as a current reference.

### Benchmark at a glance (pre-2026-08-17 benchmark — superseded, see warning above)
...
| **Best CL method found: experience replay** (3 seeds, **pre-rebuild benchmark**) | 0.861 ± 0.014 final acc., 0.094 ± 0.015 avg. forgetting (§23) |
| Joint-training upper bound (non-continual reference, **pre-rebuild benchmark**) | 0.876 acc., 0.842 F1-macro (§24) |

**Current (post-rebuild) equivalents**, for comparison, from `docs/results.md`: GNN + uniform replay reaches 0.764 final acc. / 0.202 forgetting, and the non-continual GNN joint upper bound is 0.807 acc. / 0.785 F1-macro. These are *lower* than the pre-rebuild numbers above precisely because the pre-rebuild numbers were inflated by benign memorisation (§0 of `docs/methodology-2026-08-17.md`) — the drop is evidence the rebuild worked, not regression.
