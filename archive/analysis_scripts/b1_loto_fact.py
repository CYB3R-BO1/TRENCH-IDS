#!/usr/bin/env python3
"""B1 LOTO Evaluation on FACT model.

For each transition T_n -> T_{n+1}:
1. Load checkpoint after task n (3-layer residual + replay + FACT)
...
def run_b1_transition(src_task: int, seed: int = 42) -> dict:
    ...
    model, classifier, _ = load_checkpoint(
        Path(f"runs/fact_strong/checkpoint_task_{src_task}.pt"),
        Path("data/graphs"),
        resolve_device("cuda")
    )
    ...
    for seed in [42, 1]:
        ...
        for src_task in [1, 2, 3, 4, 5]:
            ...
            try:
                ...
                target_task = src_task + 1
                target_train = load_split(Path("data/graphs"), src_task + 1, "train")
                target_test = load_split(Path("data/graphs"), src_task + 1, "test")
                
                # Use small subset for fast evaluation
                import random
                fine_tune_graphs = random.sample(target_train, min(200, len(target_train)))
                ...
                for epoch in range(3):
                    # Fine-tune for one epoch
                    model.train()
                    fine_tune_sample = random.sample(
                        load_split(Path("data/graphs"), src_task + 1, "train"),
                        min(200, len(list(load_split(Path("data/graphs"), src_task + 1, "train"))))
                    )
                    loader = GeoDataLoader(fine_tune_sample, batch_size=8, shuffle=True)
    ...
    for seed in [42, 1]:
        ...
        for src_task in [1, 2, 3, 4, 5]:
            ...
            try:
                ...
                # Fine-tune on target task
                target_task = src_task + 1
                target_train = load_split(Path("data/graphs"), target_task, "train")
                target_test = load_split(Path("data/graphs"), target_task, "test")
