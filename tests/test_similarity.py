...
def two_dataset_root(tmp_path: Path) -> Path:
    _write_csv(
        tmp_path, "NF-ToN-IoT-v2", [(100, "ddos"), (100, "ddos"), (50, "scanning")]
    )
    _write_csv(
        tmp_path,
        "NF-BoT-IoT-v2",
        [(200, "DDoS"), (200, "DDoS"), (30, "Reconnaissance")],
    )
    return tmp_path
...
TEST_CLASS_DATASETS = {"DDoS": {"ToN"}, "Scanning": {"ToN"}, "Reconnaissance": {"BoT"}}


def test_count_classes_restricts_to_allowed_datasets(two_dataset_root: Path) -> None:
    cfg = _cfg(two_dataset_root)
    counts = count_classes(cfg, list(TEST_CLASS_DATASETS), TEST_CLASS_DATASETS)
...
def test_sample_classes_restricts_to_allowed_datasets(two_dataset_root: Path) -> None:
    ...
    samples = sample_classes(cfg, list(TEST_CLASS_DATASETS), counts, rng, TEST_CLASS_DATASETS)
