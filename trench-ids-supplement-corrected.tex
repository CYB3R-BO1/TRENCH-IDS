...
\textbf{Benchmark at a glance (post-2026-08-17 quota-based rebuild):}
\begin{table}[H]
\centering
...
Total mini-graphs (\texttt{benign\_ratio=3.0}) & 10,402 (remeasured on \texttt{data/graphs}) \\
\texttt{benign\_ratio} sweep candidates & 2.0 / 3.0 / 4.0 \\
Step 3 model parameters (3-layer residual) & 622,482 (vocab PROTOCOL 5 / L7 239) \\
Test suite & 373 tests passing (conda \texttt{base}); \texttt{ruff check} reports findings, mostly in \texttt{transfer\_predictor} \\
\bottomrule
\end{tabular}
\end{adjustbox}
\end{table}
...
Sweep sets \texttt{benign\_ratio=2.0} (\texttt{data/graphs\_ratio2/}) and \texttt{benign\_ratio=4.0} (\texttt{data/graphs\_ratio4/}) are not present in this checkout, so their graph counts could not be revalidated here; the previously reported \num{78452} / \num{65378} figures are kept as-reported.
...
\begin{table}[H]
\centering
\caption{Architecture ladder. Reachable params = gradient-reachable from \texttt{fused["flow"]}.}
...
TRD-drift & 0.689 $\pm$ 0.008 & 0.363 $\pm$ 0.004 & 0.168 & Null \\
...
Additional pooled final metrics are in \texttt{eval/pooled\_final\_metrics.json}.
\end{itemize}
